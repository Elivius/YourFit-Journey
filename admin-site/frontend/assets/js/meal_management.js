...
document.getElementById('deleteBtn').addEventListener('click', function () {
    ...
    if (confirm(`Are you sure you want to delete ${selected.length} meal(s)?`)) {
        const form = document.createElement('form');
        form.method = 'POST';
        form.action = '../backend/process_delete_meal_management.php';
...
filterInput.addEventListener('input', function () {
    ...
    Array.from(rows).forEach(row => {
        const mealName = row.cells[2].textContent.toLowerCase().trim(); // Meal Name
