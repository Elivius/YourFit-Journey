...
document.getElementById('editBtn').addEventListener('click', function () {
    ...
    document.getElementById('updateName').value = cells[2].innerText;
    document.getElementById('updateProtein').value = cells[3].innerText;
    document.getElementById('updateCarbs').value = cells[4].innerText;
    document.getElementById('updateFats').value = cells[5].innerText;
    document.getElementById('updateCalories').value = cells[6].innerText;

    let idInput = document.getElementById('updateIngredientId');
    if (!idInput) {
        idInput = document.createElement('input');
        idInput.type = 'hidden';
        idInput.name = 'ingredientId';
        idInput.id = 'updateIngredientId';
...
document.getElementById('deleteBtn').addEventListener('click', function () {
    ...
    if (confirm(`Are you sure you want to delete ${selected.length} ingredient(s)?`)) {
        const form = document.createElement('form');
        form.method = 'POST';
        form.action = '../backend/process_delete_ingredient_management.php';
...
filterInput.addEventListener('input', function () {
    ...
    Array.from(rows).forEach(row => {
        const name = row.cells[2].textContent.toLowerCase().trim(); // Name
        const visible = name.includes(filterValue);
